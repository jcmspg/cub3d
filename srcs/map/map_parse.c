...
int parse_colors(char *line, t_cub_data *data)
{
	...
	if (b < 0)
	{
		ft_printf_fd(STDERR_FILENO, "Error: Invalid color format (B)\n");
		return (-1);
	}
	...
	if (ft_strncmp(line, "F ", 2) == 0)
	{
		...
		ft_printf("Parsed floor color: R=%d G=%d B=%d\n", r, g, b);
	}
	else if (ft_strncmp(line, "C ", 2) == 0)
	{
		data->textures->ceiling_color = color;
		ft_printf("Parsed ceiling color: R=%d G=%d B=%d\n", r, g, b);
	}
...
int parse_cub_file(char *filename, t_cub_data *data)
{
	...
	if (fd < 0)
	{
		ft_printf_fd(STDERR_FILENO, "Error opening file for texture parsing\n");
	...
	while ((line = get_next_line(fd)))
	{
		...
		{
			ft_printf("DEBUG: Parsing texture line: %s", line);
			if (parse_textures(line, data) != 0)
			{
				ft_printf_fd(STDERR_FILENO, "Error: Failed to parse texture line\n");
				free(line);
				close(fd);
		...
		{
			ft_printf("DEBUG: Parsing color line: %s", line);
			if (parse_colors(line, data) != 0)
			{
				ft_printf_fd(STDERR_FILENO, "Error: Failed to parse color line\n");
				free(line);
				close(fd);
				return ERR_TEXTURE_PARSE;
			}
	...
	ft_printf("DEBUG: Map parsing complete!\n");
	return ERR_NO_ERROR;
}
