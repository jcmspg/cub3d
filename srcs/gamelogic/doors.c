...
static void toggle_door(t_cub_data *data, t_door *door) {
  int player_map_x = from_fixed32(data->player->x);
  int player_map_y = from_fixed32(data->player->y);
  ...
  if (door->is_open || door->is_opening) {
    ...
    door->is_opening = false;
    door->is_open =
        false; // Immediately treat as solid for collision? Maybe wait.
  } else {
...
void interact_doors(t_cub_data *data) {
  ...
  for (dist = 0.5f; dist <= 1.5f; dist += 0.5f) {
    check_x = fixed32_add(data->player->x,
                          fixed32_mul(data->player->dir_x, to_fixed32(dist)));
    check_y = fixed32_add(data->player->y,
                          fixed32_mul(data->player->dir_y, to_fixed32(dist)));
