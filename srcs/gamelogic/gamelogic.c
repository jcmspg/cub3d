...
static void update_head_bob(t_cub_data *data) {
  ...
  data->player->bob_offset =
      (int)(sinf(data->player->bob_phase) * BOB_AMPLITUDE);
}
...
void update_game_logic(t_cub_data *data) {
  ...
  data->player->move_speed =
      to_fixed32(PLAYER_SPEED * speed_mult * (data->fps.delta_time / 16.67f));
  data->player->rotate_speed =
      to_fixed32(ROTATE_SPEED * (data->fps.delta_time / 16.67f));
