  ...
  while (y < view_cells + 2) {
    ...
    while (x < view_cells + 2) {
      ...
      if (draw_x < minimap_x + minimap_size && draw_x + cell_size > minimap_x &&
        ...
        draw_minimap_cell_at(data, start_cell_x + x, start_cell_y + y, draw_x,
                             draw_y, cell_size);
      x++;
    }
    y++;
...
void draw_minimap_player(t_cub_data *data, int cell_size) {
  ...
  player_x =
      data->hud->minimap_x + (int)(from_fixed32(data->player->x) * cell_size);
  player_y =
      data->hud->minimap_y + (int)(from_fixed32(data->player->y) * cell_size);
  // Draw player dot (small square)
  size = cell_size / 2;
  if (size < 3)
    size = 3;
