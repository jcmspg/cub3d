...
int	init_hud(t_cub_data *data)
{
	...
	// Minimap position is calculated dynamically in draw_hud_minimap
	// Initialize background as solid color (texture can be added later)
...
int	cleanup_hud(t_hud *hud)
{
	...
	if (hud->bg_img)
	{
		// mlx_destroy_image would be called here
		hud->bg_img = NULL;
	}
	free(hud);
	return (ERR_NO_ERROR);
...
void	draw_hud_minimap(t_cub_data *data)
{
	...
	if (!data->map || !data->map->map_array)
		return ;
	// Minimap is a square that fits HUD height (minus padding and border)
	minimap_size = data->hud->height - (HUD_MINIMAP_PADDING * 2) - HUD_BORDER_WIDTH;
	cell_size = HUD_MINIMAP_CELL;
	view_cells = minimap_size / cell_size; // how many cells fit in view
	// Position minimap on the RIGHT side of HUD
	...
	// Draw rays on minimap (before player so player dot is on top)
	draw_hud_rays(data, minimap_x, minimap_y, minimap_size, cell_size);
	// Draw player dot in center of minimap
	draw_minimap_player_centered(data, minimap_x, minimap_y, minimap_size, cell_size);
}
							...
							int screen_x, int screen_y, int cell_size)
{
	...
	if (map_x < 0 || map_x >= data->map->width
		...
		color = 0x111111; // Dark gray for out of bounds
	else
	{
...
void	draw_minimap_player(t_cub_data *data, int cell_size)
{
	...
	player_x = data->hud->minimap_x
		+ (int)(from_fixed32(data->player->x) * cell_size);
	player_y = data->hud->minimap_y
		+ (int)(from_fixed32(data->player->y) * cell_size);
	// Draw player dot (small square)
	size = cell_size / 2;
	if (size < 3)
		size = 3;
...
void	render_hud(t_cub_data *data)
{
	...
	// Future: draw health, ammo, weapon, etc.
}
