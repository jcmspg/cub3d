...
int	get_wall_color(t_ray *ray, t_textures *textures)
{
	...
	if (ray->side == 0)
	{
		// Hit vertical wall - ray going East sees West face, ray going West sees East face
		if (ray->step_x > 0)
	...
	{
		// Hit horizontal wall - ray going South sees North face, ray going North sees South face
		if (ray->step_y > 0)
