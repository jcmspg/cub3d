...
void	calculate_ray_dir(t_cub_data *data, t_ray *ray, int x)
{
	...
	camera_x = fixed32_sub(
			fixed32_div(
				fixed32_mul(to_fixed32(2.0f), to_fixed32((float)x)),
...
void	init_dda(t_cub_data *data, t_ray *ray)
{
	...
	if (fixed32_abs(ray->dir_x) < to_fixed32(0.0001f))
		...
		ray->delta_dist_x = fixed32_abs(
				fixed32_div(to_fixed32(1.0f), ray->dir_x));
	if (fixed32_abs(ray->dir_y) < to_fixed32(0.0001f))
		ray->delta_dist_y = to_fixed32(10000.0f);
	else
		ray->delta_dist_y = fixed32_abs(
				fixed32_div(to_fixed32(1.0f), ray->dir_y));
	...
	else
	{
		...
		ray->side_dist_x = fixed32_mul(
				fixed32_sub(to_fixed32(1.0f), frac_x), ray->delta_dist_x);
	}
	if (ray->dir_y < 0)
	{
		ray->step_y = -1;
	...
	else
	{
		...
		ray->side_dist_y = fixed32_mul(
				fixed32_sub(to_fixed32(1.0f), frac_y), ray->delta_dist_y);
	}
}
...
int	perform_dda(t_cub_data *data, t_ray *ray)
{
	...
	while (step < max_steps)
	{
		...
		if (data->map->map_array[ray->map_y * data->map->width + ray->map_x]
			== '1'
